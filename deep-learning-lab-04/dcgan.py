...
def discriminator(x, isTrain=True, reuse=False):
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        # output layer
        flatten_out = tf.layers.flatten(lrelu_2)
        out = tf.layers.dense(flatten_out, 1, activation=None)
...
def generator(z, isTrain=True):
    with tf.variable_scope('generator'):
        ...
        # 2nd hidden layer
        conv_2 = tf.layers.conv2d_transpose(lrelu_, 256, [4, 4], strides=(2, 2), padding='same')
        lrelu_2 = lrelu(tf.layers.batch_normalization(conv_2, training=isTrain))

        # 3rd hidden layer
        conv_3 = tf.layers.conv2d_transpose(lrelu_2, 128, [4, 4], strides=(2, 2), padding='same')
        lrelu_3 = lrelu(tf.layers.batch_normalization(conv_3, training=isTrain))

        # output layer
        conv_4 = tf.layers.conv2d_transpose(lrelu_3, 1, [4, 4], strides=(2, 2), padding='same')
        out = tf.tanh(conv_4)
...
D_loss_real = loss_func(D_real_logits, tf.ones_like(D_real_logits) * 0.9)
D_loss_fake = loss_func(D_fake_logits, tf.zeros_like(D_real_logits))
...
train_set = tf.image.per_image_standardization(tf.image.resize_images(mnist.train.images, [32, 32])).eval()
