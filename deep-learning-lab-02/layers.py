...
class FC(Layer):
  ...
  def forward(self, inputs):
    ...
    """
    self.x = inputs
    return np.dot(inputs, self.weights.T) + self.bias.T 
  ...
  def backward_params(self, grads):
    ...
    grad_weights = np.dot(grads.T, self.x)
...
class ReLU(Layer):
  ...
  def forward(self, inputs):
    ...
    self.scores = np.maximum(inputs,0)
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    logprobs = np.log(probs)  # N x 
    
    return - 1 / N * np.sum(logprobs * y)
